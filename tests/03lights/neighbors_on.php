...
if(HasPlugin("NullLights")){
    ...
    $on_min = round(SecondsToMinutes(WemoTime::NeighborsOnTime($light3)));
    $off_min = round(SecondsToMinutes(WemoTime::NeighborsOffTime($light5)));
    $off_min_zero = round(SecondsToMinutes(WemoTime::NeighborsOffTime($light3)));
    ...
    if($on_min == 5 && $off_min_zero == 0 && $on_min == $off_min && $off_min == $off_min_old) $tests['running'][$test_name] = "Pass";
        ...
        $tests['running'][$test_name]["WemoTime::NeighborsOnTime"] = ($on_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::NeighborsOffTime"] = ($off_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::NeighborsOffTime(false positive)"] = ($off_min_zero == 0) ? "Pass" : "-Fail";
