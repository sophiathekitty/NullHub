...
if(HasPlugin("NullLights")){
    ...
    $on_min = round(SecondsToMinutes(WemoTime::RoomMoodOnTime($light6)));
    $off_min = round(SecondsToMinutes(WemoTime::RoomMoodOffTime($light2)));
    $off_min_zero = round(SecondsToMinutes(WemoTime::RoomMoodOffTime($light6)));
    ...
    if($on_min == 5 && $off_min_zero == 0 && $on_min == $off_min && $on_min == $on_min_old && $off_min == $off_min_old) $tests['running'][$test_name] = "Pass";
        ...
        $tests['running'][$test_name]["WemoTime::RoomMoodOnTime"] = ($on_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::RoomMoodOffTime"] = ($off_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::RoomMoodOffTime(false positive)"] = ($off_min_zero == 0) ? "Pass" : "-Fail";
