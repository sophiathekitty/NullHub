...
if(HasPlugin("NullLights")){
    ...
    $on_min = round(SecondsToMinutes(WemoTime::RoomLampOnTime($light1)));
    $off_min = round(SecondsToMinutes(WemoTime::RoomLampOffTime($light4)));
    $off_min_zero = round(SecondsToMinutes(WemoTime::RoomLampOffTime($light1)));
    ...
    if($on_min == 5 && $off_min_zero == 0 && $on_min == $off_min && $on_min == $on_min_old && $off_min == $off_min_old) $tests['running'][$test_name] = "Pass";
        ...
        $tests['running'][$test_name]["WemoTime::RoomLampOnTime"] = ($on_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::RoomLampOffTime"] = ($off_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::RoomLampOffTime(false positive)"] = ($off_min_zero == 0) ? "Pass" : "-Fail";
