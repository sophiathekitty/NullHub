...
if(HasPlugin("NullLights")){
    ...
    $on_min = round(WemoTime::OnMinutes($light1));
    $off_min_zero = round(WemoTime::OffMinutes($light1));
    $off_min = round(WemoTime::OffMinutes($light2));
    ...
    if($on_min == 5 && $off_min_zero == 0 && $on_min == $off_min && $on_min == $on_min_old && $off_min == $off_min_old) $tests['running'][$test_name] = "Pass";
        ...
        $tests['running'][$test_name]["WemoTime::OnMinutes"] = ($on_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::OffMinutes"] = ($off_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["WemoTime::OffMinutes(false positive)"] = ($off_min == 0) ? "Pass" : "-Fail";
