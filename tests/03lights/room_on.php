...
if(HasPlugin("NullLights")){
    ...
    $on_min = round(SecondsToMinutes(RoomLightTime::RoomOnTime($light1)));
    $off_min_zero = round(SecondsToMinutes(RoomLightTime::RoomOffTime($light1)));
    $off_min = round(SecondsToMinutes(RoomLightTime::RoomOffTime($light4)));
    ...
    if($on_min == 5 && $off_min_zero == 0 && $on_min == $off_min && $off_min == $off_min_old) $tests['running'][$test_name] = "Pass";
        ...
        $tests['running'][$test_name]["RoomLightTime::RoomOnTime"] = ($on_min == 5) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["RoomLightTime::RoomOffTime(false positive)"] = ($off_min_zero == 0) ? "Pass" : "-Fail";
        $tests['running'][$test_name]["RoomLightTime::RoomOffTime"] = ($off_min == 5) ? "Pass" : "-Fail";
